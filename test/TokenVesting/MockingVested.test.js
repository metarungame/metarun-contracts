...
describe("Token Vesting Math underflow/overflow", function () {
  this.beforeEach(async function () {
    this.TokenVestingMathMock = await ethers.getContractFactory("TokenVestingMockVested");
  ...
  describe("Vesting amount calculations", function () {
    it("should throw underflow when vested is negative amount", async function () {
      await this.vesting.setCurrentBlockTime(this.cliff);
      await this.vesting.setVestedAmount(10);
      await this.vesting.release(this.beneficiaryAddress);
      await this.vesting.setVestedAmount(5);
      const dummyRelease = this.vesting.release(this.beneficiaryAddress);
      await expect(dummyRelease).to.be.revertedWith(
