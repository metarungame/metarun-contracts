...
describe("MysteryBoxSale", function () {
  ...
  beforeEach(async function () {
    ...
    await this.busdToken.connect(this.buyer).approve(this.mysteryBoxSale.address, this.mysteryBoxPrice);
  });
  ...
  describe("Buy", function () {
    it("should increase sale contract's BUSD balance", async function () {
      ...
      await this.mysteryBoxSale.connect(this.buyer).buy("Seller");
    ...
    it("should emit MysteryBoxBought event", async function () {
      const tokenId = this.mysteryBoxKind << 16;
      attempt = this.mysteryBoxSale.connect(this.buyer).buy("Seller");
      await expect(attempt).to.emit(this.mysteryBoxSale, "MysteryBoxBought").withArgs(this.buyer.address, tokenId, "Seller");
    });
  ...
  describe("Set Mystery Box Price", function () {
    ...
    it("msg.sender with SETTER_ROLE can set price", async function () {
      expect(await this.mysteryBoxSale.mysteryBoxPrice()).to.be.eq(this.mysteryBoxPrice);
      await this.mysteryBoxSale.connect(this.deployer).setTicketKindPrice(this.newPrice);

      expect(await this.mysteryBoxSale.mysteryBoxPrice()).to.be.eq(this.newPrice);
    });
    it("should revert if msg.sender without SETTER_ROLE role", async function () {
      attempt = this.mysteryBoxSale.connect(this.buyer).setTicketKindPrice(this.newPrice);
