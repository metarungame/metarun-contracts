...
describe("Metarun Token Vesting", function () {
  ...
  describe("create vesting", function () {
    beforeEach("createVesting", async function () {
      await this.vesting.createVesting(this.deployer.address, start, interval, duration, balance);
    });
    ...
    describe("reverts", function () {
      it("when interval > duration", async function () {
        await expect(this.vesting.createVesting(this.deployer.address, start, interval + 1, interval, balance)).to.be.revertedWith(
          "TokenVesting #createVesting: interval cannot be bigger than duration"
        );
      });

      it("when interval = 0", async function () {
        await expect(this.vesting.createVesting(this.deployer.address, start, 0, duration, balance)).to.be.revertedWith(
          "TokenVesting #createVesting: interval must be greater than 0"
        );
      });

      it("when current balance > 0", async function () {
        await expect(this.vesting.createVesting(this.deployer.address, start, interval, duration, balance)).to.be.revertedWith(
    ...
    const checkVesting = function (intervalsVested, alreadyReleased) {
      describe(`should vested ${intervalsVested} intervals`, async function () {
        ...
        it("releasableAmount", async function () {
          ...
          const expectedReleasableAmount = balance.mul(intervalsVestedBN.sub(alreadyReleasedBN)).div(durationBN.div(intervalBN));
          expect(releasableAmount).to.equal(expectedReleasableAmount);
        });
      });
    };

    const releaseAndCheckBalances = function (intervalsVested, alreadyReleased) {
      ...
      context("postpone vesting", function () {
        describe("reverts", function () {
          ...
          it("new start is before old start", async function () {
            await expect(this.vesting.postponeVesting(start)).to.be.revertedWith(
